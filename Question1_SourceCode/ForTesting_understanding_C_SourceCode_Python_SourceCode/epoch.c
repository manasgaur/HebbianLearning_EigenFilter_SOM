...
typedef struct neuron_s
   { double output;        /* sigmoid(receptive_field_of_neuron) */
     ...
     double bias;
     double bupdates;
     double bp;	          /* In this implementation, we're NOT folding
...
typedef struct network_s
   { /* Following are the counts of the number of elements in each
     ...
     double *first_layer_weights;
		double *first_layer_wupdates;
		double *first_layer_wp;  /* Weights FROM input layer TO hidden layer  */
     double *second_layer_weights;
		double *second_layer_wupdates; 
		double *second_layer_wp;	/* Weights FROM hidden layer TO output layer */
   } network_t;
...
{  *network                        = (network_t*)malloc(sizeof(network_t));
 ...
 (*network)->first_layer_wp = (double*)malloc(sizeof(double)*hidden_nodes*output_nodes);
  (*network)->second_layer_wupdates = (double*)malloc(sizeof(double)*hidden_nodes*output_nodes);
 (*network)->second_layer_wp = (double*)malloc(sizeof(double)*hidden_nodes*output_nodes);
...
{ free((*network)->second_layer_weights);
  free((*network)->first_layer_weights);
	...
	free((*network)->first_layer_wp);
	free((*network)->second_layer_wp);
...
{ int j,k;
  ...
  for (j=0; j<network->output_neuron_count; j++)
      { for (k=0; k<network->hidden_neuron_count; k++)
             network->second_layer_wupdates[j*network->hidden_neuron_count+k]=( (network->second_layer_wp[j*network->hidden_neuron_count+k])+alpha*(network->second_layer_wupdates[j*network->hidden_neuron_count+k]));
      
        network->output_layer[j].bupdates = ((network->output_layer[j].bp)+alpha*(network->output_layer[j].bupdates));

      }

  for (j=0; j<network->hidden_neuron_count; j++)
      { for (k=0; k<network->input_node_count; k++)
             network->first_layer_wupdates[j*network->input_node_count+k] = ((network->first_layer_wp[j*network->input_node_count+k])+alpha*(network->first_layer_wupdates[j*network->input_node_count+k]));

       network->hidden_layer[j].bupdates = ((network->hidden_layer[j].bp)+alpha*(network->hidden_layer[j].bupdates));

      }

}
void batchupdate(network_ptr network, double learning_rate)
...
{ int j,k;
  ...
  for (j=0; j<network->output_neuron_count; j++)
      { for (k=0; k<network->hidden_neuron_count; k++)
             network->second_layer_wp[j*network->hidden_neuron_count+k]+=( (learning_rate *
                                                                                network->output_layer[j].gradient *
                                                                                 network->hidden_layer[k].output));
      
        network->output_layer[j].bp+= (learning_rate * network->output_layer[j].gradient);

      }

  for (j=0; j<network->hidden_neuron_count; j++)
      { for (k=0; k<network->input_node_count; k++)
             network->first_layer_wp[j*network->input_node_count+k]+= ((learning_rate *
                                                                                network->hidden_layer[j].gradient *
                                                                                network->input_layer[k]));

       network->hidden_layer[j].bupdates += ((learning_rate * network->hidden_layer[j].gradient));
...
{ int c,i,j,k;
  ...
  /* Set up neural network */
   malloc_network(2,3, 2, &my_network); /* Two input nodes, two hidden nodes, one output node */
...
double mean1,mean2,variance1,variance2;
   ...
   for (c=0; (fabs(newerror-error)>0.000001)||(error>0.0001); c++)
	...
	for (j=0; j<my_network->output_neuron_count; j++)//wupdates of secondlayer are made zero
      { for (k=0; k<my_network->hidden_neuron_count; k++)
             my_network->second_layer_wp[j*my_network->hidden_neuron_count+k]=0.0;}
	for (j=0; j<my_network->hidden_neuron_count; j++)//wupdates of the first layer are made zero
      { for (k=0; k<my_network->input_node_count; k++)
             my_network->first_layer_wp[j*my_network->input_node_count+k]=0.0;}
	for (j=0; j<my_network->hidden_neuron_count; j++){//bupdates of hidden neurons are made zero
          my_network->hidden_layer[j].bp =0.0;}
	for (j=0; j<my_network->output_neuron_count; j++){//bupdates of output layer are made zero
          my_network->output_layer[j].bp =0.0;}

        for(random_index=0;random_index<5000;random_index++)
           { X[0][0] = foo1[random_index][0];
             X[0][1] = foo1[random_index][1];
		compute_network_outputs(X[0], my_network);                     /* feed inputs forward  */
        compute_network_gradients(D[0], my_network);
	batchupdate(my_network, 0.0025);
           } 
        for(random_index=0;random_index<5000;random_index++)
           { X[0][0] = bar1[random_index][0];
             X[0][1] = bar1[random_index][1];
		compute_network_outputs(X[0], my_network);                     /* feed inputs forward  */
        compute_network_gradients(D[1], my_network);
	batchupdate(my_network, 0.0025);
           }
        for(random_index=0;random_index<5000;random_index++)
		...
		compute_network_outputs(X[0], my_network);                     /* feed inputs forward  */
        compute_network_gradients(D[2], my_network);
	batchupdate(my_network, 0.0025);
           } 
        for(random_index=0;random_index<5000;random_index++)
		...
		compute_network_outputs(X[0], my_network);                     /* feed inputs forward  */
        compute_network_gradients(D[3], my_network);
	batchupdate(my_network, 0.0025);
           }
   ...
   /*file_network_parameters(my_network);*/
	...
	printf("The number of epoches is : %d\n",points);
	/*printf("The error %f\n",error); 
	printf("The new error %f\n",newerror);*/
