...
{ int c,i,j,k;
  ...
  /* Set up neural network */
   malloc_network(2,14, 2, &my_network); /* Two input nodes, fourteen hidden nodes, two output node */
...
double variance1,variance2;
		...
		mean2=mean2/20000;
		for(j=0;j<5000;j++){
			variance1+=(foo[j][0]-mean1)*(foo[j][0]-mean1);
			variance2+=(foo[j][1]-mean2)*(foo[j][1]-mean2);}
		for(j=0;j<5000;j++){
			variance1+=(bar[j][0]-mean1)*(bar[j][0]-mean1);
			variance2+=(bar[j][1]-mean2)*(bar[j][1]-mean2);}
		for(j=0;j<5000;j++){
			variance1+=(bonk[j][0]-mean1)*(bonk[j][0]-mean1);
			variance2+=(bonk[j][1]-mean2)*(bonk[j][1]-mean2);}
		for(j=0;j<5000;j++){
			variance1+=(baz[j][0]-mean1)*(baz[j][0]-mean1);
			variance2+=(baz[j][1]-mean2)*(baz[j][1]-mean2);}
		variance1=sqrt(variance1/19999);
		variance2=sqrt(variance2/19999);
		for(j=0;j<5000;j++){
			foo[j][0]-=mean1;
			foo[j][0]=foo[j][0]/variance1;
			foo[j][1]-=mean2;
			foo[j][1]=foo[j][1]/variance2;	}
		for(j=0;j<5000;j++){
			bar[j][0]-=mean1;
			bar[j][0]=bar[j][0]/variance1;
			bar[j][1]-=mean2;
			bar[j][1]=bar[j][1]/variance2;}
		for(j=0;j<5000;j++){
			bonk[j][0]-=mean1;
			bonk[j][0]=bonk[j][0]/variance1;
			bonk[j][1]-=mean2;
			bonk[j][1]=bonk[j][1]/variance2;}
		for(j=0;j<5000;j++){
			baz[j][0]-=mean1;
			baz[j][0]=baz[j][0]/variance1;
			baz[j][1]-=mean2;
			baz[j][1]=baz[j][1]/variance2;}
   ...
   for (c=0; (fabs(newerror-error)>0.000001)||(error>0.0001); c++)
	...
	random_class = 1 + (int)trunc(rng_uniform(rng_one, 0.0,   3.9999));
        ...
        if (random_class == 1)
           { X[0][0] = foo[random_index][0];
             X[0][1] = foo[random_index][1];
           } else
        if (random_class == 2)
           { X[0][0] = bar[random_index][0];
             X[0][1] = bar[random_index][1];
           } else
        if (random_class == 3)
           { X[0][0] = baz[random_index][0];
             X[0][1] = baz[random_index][1];
           } else
        if (random_class == 4)
           { X[0][0] = bonk[random_index][0];
             X[0][1] = bonk[random_index][1];
           };
...
printf("The number of pattern presentations is:%d\n",points);
